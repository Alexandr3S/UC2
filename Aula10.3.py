...
# Criando o DataFrame ocorrencias de toda a série histórica
...
if len(df_cvli_outliers_superiores) != 0 or len(df_cvli_outliers_inferiores) != 0:
    ...
    # posição 01: Gráfico dos CVLI
    df_rec_veiculo_order = df_cvli.sort_values(by='cvli',ascending=True)
    plt.subplot(2,2,1)
    plt.title('Acumulado dos Crimes Violentos Letais Intencionais - CVLI')
    plt.bar(df_rec_veiculo_order['aisp'].astype(str),df_rec_veiculo_order['cvli'])
